...
// (needs HTMLCanvasElement.getContext, which jsdom doesn't implement — see
// docs/accessibility-review.md), so every component test's a11y check
// silently skips contrast. This runs axe in a real browser instead, where
// contrast checking actually works.
test.describe("accessibility (real browser axe scan)", () => {
  ...
  test("admin dashboard has no automatically detectable a11y violations", async ({ page }) => {
    ...
    // catching the page before its <h1>/<title> have committed).
