import { ALLOWED_IMAGE_HOSTS, verifyAdminToken } from "@outfit-builder/contracts";
...
function buildCsp(nonce: string): string {
  ...
  return [
    ...
    // nonce-only style-src would silently break those.
    "style-src 'self' 'unsafe-inline'",
    // Same host list product image validation uses (packages/contracts/src/
    // domain/image-hosts.ts) — an image host that isn't in that list would
    // fail to load here regardless, so this can't silently drift from it.
    `img-src 'self' data: ${ALLOWED_IMAGE_HOSTS.map((host) => `https://${host}`).join(" ")}`,
    "font-src 'self' data:",
