...
function buildCsp(nonce: string): string {
  // Client Components fetch apps/api directly for outfit save/delete
  // (docs/performance-review.md), so connect-src must allow that origin or
  // those requests get silently blocked by the browser.
  ...
  return [
    ...
    // nonce-only style-src would silently break those. Documented
    // tradeoff (docs/security-review.md), not an oversight.
...
export async function proxy(request: NextRequest) {
  ...
  // Forwarded so Next.js can stamp this nonce onto the scripts it injects
  // itself (RSC bootstrap/hydration) — it auto-detects the nonce from the
  // outgoing Content-Security-Policy header, no manual threading needed
  // for this app since it has no custom inline <script> tags of its own.
  ...
  if (request.nextUrl.pathname.startsWith("/admin") && request.nextUrl.pathname !== "/admin/login") {
    ...
    if (!token || !secret) {
      return withSecurityHeaders(NextResponse.redirect(new URL("/admin/login", request.url)), nonce);
    }
    ...
    try {
      ...
      // security boundary (docs/07-security.md "Auth").
  ...
  return withSecurityHeaders(NextResponse.next(requestInit), nonce);
}
