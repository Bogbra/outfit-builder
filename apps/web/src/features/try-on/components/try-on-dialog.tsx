...
import { TRY_ON_ELIGIBLE_CATEGORIES } from "@outfit-builder/contracts";
...
export function TryOnDialog({ items }: TryOnDialogProps) {
  ...
  // fashn.ai's tryon-v1.6 model only supports garments (see category.ts) —
  // shoes/bags/accessories are excluded here rather than sent to a model
  // that isn't built for them (try-on-repository.ts enforces the same
  // filter server-side, since this is only a UX convenience, not the
  // security boundary).
  const eligibleItems = useMemo(
    () => items.filter((entry) => TRY_ON_ELIGIBLE_CATEGORIES.includes(entry.product.category)),
    [items],
  );
  const excludedCount = items.length - eligibleItems.length;
  ...
  async function handleTryOn() {
    ...
    try {
      ...
      const response = await startTryOn({
        photo,
        items: eligibleItems.map((entry) => ({ productId: entry.product.id, category: entry.product.category })),
      });
  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button variant="secondary" disabled={eligibleItems.length === 0}>
          Try it on
        </Button>
      </DialogTrigger>
      <DialogContent
        title="Try it on"
        description="Upload a photo of yourself to see this outfit on you. Your photo is sent to fashn.ai, a third-party AI service, to generate the result — this application does not store the original upload. AI-generated composite, not a real garment fitting."
      >
        <div className="flex flex-col gap-4">
          {!result?.resultImageUrl && excludedCount > 0 && (
            <p className="text-base text-muted-foreground">
              {excludedCount === 1
                ? "1 item in this outfit isn't shown in the try-on preview — shoes, bags and accessories aren't supported yet."
                : `${excludedCount} items in this outfit aren't shown in the try-on preview — shoes, bags and accessories aren't supported yet.`}
            </p>
          )}
          ...
          {isProcessing && (
            <div className="flex items-center gap-3" role="status" aria-live="polite">
              <Badge variant="info">
                Trying on {result?.step ?? 1} of {result?.totalSteps ?? eligibleItems.length}
              </Badge>
