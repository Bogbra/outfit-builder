...
// averageOutfitPrice has no currency of its own — formatted as EUR since
// the catalog is single-currency for this MVP (docs/open-questions.md).
export function AnalyticsCards({ analytics }: AnalyticsCardsProps) {
  const cards: { label: string; value: string; capitalize?: boolean }[] = [
    ...
    { label: "Most used category", value: analytics.mostUsedCategory ?? "—", capitalize: true },
    {
      ...
      value: analytics.averageOutfitPrice !== null ? formatPrice(analytics.averageOutfitPrice, "EUR") : "—",
    },
  ];
