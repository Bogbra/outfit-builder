...
describe("AdminProductSearchForm", () => {
  ...
  it("drops the page param but keeps sort params when re-searching", () => {
    currentSearchParams = new URLSearchParams({ sortBy: "priceMinor", sortDirection: "asc", page: "3" });
    render(<AdminProductSearchForm />);
    fireEvent.change(screen.getByRole("searchbox", { name: "Search products" }), {
      target: { value: "denim" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Search" }));
    expect(pushMock).toHaveBeenCalledWith("/admin/products?sortBy=priceMinor&sortDirection=asc&search=denim");
  });
