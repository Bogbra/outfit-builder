...
export function createApp(): Express {
  ...
  // graphql-armor above limits how expensive a single query can be; this
  // limits how many queries a client can send at all — every REST route in
  // this API has one, /graphql shouldn't be the one unlimited surface.
  const graphqlLimiter = createRateLimiter({
    windowMs: env.GRAPHQL_RATE_LIMIT_WINDOW_SECONDS * 1000,
    limit: env.GRAPHQL_RATE_LIMIT_MAX_REQUESTS,
  });
  app.use(yoga.graphqlEndpoint, graphqlLimiter, yoga);
