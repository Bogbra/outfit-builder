...
// Reads the JWT payload without verifying the signature — only ever used
// here to pull the jti out of a token this same test just legitimately
// obtained via login(), so it can mutate that session's row directly.
...
describeIfDb("admin API", () => {
  ...
  describe("POST /api/auth/logout", () => {
    ...
    it("rejects a token whose session has passed its expiresAt, even though the JWT itself is still valid", async () => {
      const token = await login();
      const { jti } = decodeJwtPayload(token) as { jti: string };
      await prisma.adminSession.update({ where: { id: jti }, data: { expiresAt: new Date(Date.now() - 1000) } });
  ...
  describe("admin product routes", () => {
    ...
    it("creates, reads and updates a product end to end", async () => {
      ...
      const updateResponse = await request(app)
        ...
        .set("Authorization", `Bearer ${token}`)
        .send({ isActive: false, priceMinor: 2500 });
      expect(updateResponse.status).toBe(200);
      expect(updateResponse.body.isActive).toBe(false);
      expect(updateResponse.body.priceMinor).toBe(2500);
  ...
  describe("GET /api/admin/analytics", () => {
    ...
    it("returns analytics for an authenticated admin", async () => {
      ...
      expect(response.body).toHaveProperty("averageOutfitPriceMinor");
