...
vi.mock("../lib/fashn-client.js", () => ({
  ...
  FashnContractError: class FashnContractError extends Error {},
  FashnNotConfiguredError: class FashnNotConfiguredError extends Error {},
}));

const { createApp } = await import("../app.js");
const { prisma } = await import("../lib/prisma.js");
const { startTryOnStep, getTryOnStepStatus, FashnApiError, FashnContractError } = await import(
  "../lib/fashn-client.js"
);
...
describeIfDb("try-on REST API", () => {
  ...
  beforeAll(async () => {
    const [top, bottom, shoes] = await Promise.all([
      ...
      prisma.product.findFirstOrThrow({ where: { category: "shoes", isActive: true } }),
    ]);
    topProductId = top.id;
    bottomProductId = bottom.id;
    shoesProductId = shoes.id;
  });
  ...
  it("returns 502, not a raw 500, when the initial fashn.ai call throws a contract error", async () => {
    startTryOnStepMock.mockRejectedValueOnce(new FashnContractError("fashn.ai run response did not include a prediction id"));
  ...
  it("surfaces (doesn't silently retry) an unexpected non-fashn.ai error while polling", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-unexpected");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: topProductId, category: "top" }] });

    // A bug (e.g. a TypeError), not a documented fashn.ai error type —
    // must not be absorbed into the transient-poll-failure retry path.
    getTryOnStepStatusMock.mockRejectedValueOnce(new TypeError("something in this process is broken"));
    const pollResponse = await request(app).get(`/api/try-on/${createResponse.body.id}`);

    expect(pollResponse.status).toBe(500);
    const stored = await prisma.tryOnRequest.findUniqueOrThrow({ where: { id: createResponse.body.id } });
    expect(stored.status).toBe("processing");
    expect(stored.pollFailureCount).toBe(0);
  });

  it("surfaces (doesn't hide as a clean failure) an unexpected non-fashn.ai error starting the next chain step", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-midchain");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({
        photo: PHOTO,
    ...
    // Step 1 completes, so advanceTryOnRequest tries to start step 2 —
    // that call throws a bug, not a documented fashn.ai error type, and
    // must not be swallowed into a generic "failed" try-on status.
    getTryOnStepStatusMock.mockResolvedValueOnce({
      status: "completed",
      outputUrl: "https://cdn.fashn.ai/result-step1.png",
    });
    startTryOnStepMock.mockRejectedValueOnce(new TypeError("something in this process is broken"));
    const pollResponse = await request(app).get(`/api/try-on/${createResponse.body.id}`);
  ...
  it("excludes shoes (fashn.ai's model doesn't support them) and tries on only the eligible items", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-shoes-excluded");
    const response = await request(app)
      .post("/api/try-on")
      .send({
        photo: PHOTO,
        items: [
          { productId: topProductId, category: "top" },
          { productId: shoesProductId, category: "shoes" },
        ],
      });

    expect(response.status).toBe(201);
    expect(response.body).toMatchObject({ status: "processing", step: 1, totalSteps: 1 });
    const stored = await prisma.tryOnRequest.findUniqueOrThrow({ where: { id: response.body.id } });
    expect(stored.items).toEqual([expect.objectContaining({ category: "top" })]);
  });

  it("rejects an outfit made only of categories fashn.ai's model doesn't support, with 422", async () => {
    const response = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: shoesProductId, category: "shoes" }] });
  ...
  it("keeps a try-on request processing across a single transient polling failure", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-transient");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: topProductId, category: "top" }] });

    getTryOnStepStatusMock.mockRejectedValueOnce(new FashnApiError("network blip"));
    const firstPoll = await request(app).get(`/api/try-on/${createResponse.body.id}`);
    expect(firstPoll.status).toBe(200);
    expect(firstPoll.body.status).toBe("processing");

    // Recovers: a successful poll after a transient failure doesn't carry
    // the failure count forward into a later, unrelated failure streak.
    getTryOnStepStatusMock.mockResolvedValueOnce({ status: "processing" });
    const secondPoll = await request(app).get(`/api/try-on/${createResponse.body.id}`);
    expect(secondPoll.status).toBe(200);
    expect(secondPoll.body.status).toBe("processing");

    const stored = await prisma.tryOnRequest.findUniqueOrThrow({ where: { id: createResponse.body.id } });
    expect(stored.pollFailureCount).toBe(0);
  });

  it("fails a try-on request after repeated transient polling failures", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-flaky");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: topProductId, category: "top" }] });

    getTryOnStepStatusMock.mockRejectedValue(new FashnApiError("fashn.ai status request failed with status 503", 503));

    const polls = [];
    for (let i = 0; i < 5; i++) {
      polls.push(await request(app).get(`/api/try-on/${createResponse.body.id}`));
    }
    const lastPoll = polls[polls.length - 1]!;

    expect(lastPoll.status).toBe(200);
    expect(lastPoll.body.status).toBe("failed");
  });

  it("fails a try-on request immediately on a permanent (4xx) fashn.ai error, without retrying", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-permanent");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: topProductId, category: "top" }] });

    getTryOnStepStatusMock.mockRejectedValueOnce(new FashnApiError("fashn.ai status request failed with status 401", 401));
    const pollResponse = await request(app).get(`/api/try-on/${createResponse.body.id}`);
  ...
  it("fails a try-on request immediately on a fashn.ai contract error, without retrying", async () => {
    startTryOnStepMock.mockResolvedValueOnce("prediction-contract");
    const createResponse = await request(app)
      .post("/api/try-on")
      .send({ photo: PHOTO, items: [{ productId: topProductId, category: "top" }] });

    getTryOnStepStatusMock.mockRejectedValueOnce(new FashnContractError("unexpected response shape"));
    const pollResponse = await request(app).get(`/api/try-on/${createResponse.body.id}`);
