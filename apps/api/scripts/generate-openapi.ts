...
// so it can't silently drift out of sync with the API. Run with
...
const analyticsResponse = registry.register(
  ...
  z
    .object({
      ...
      averageOutfitPriceMinor: z.number().int().nullable(),
    })
...
registry.registerPath({
  ...
  operationId: "logout",
  summary: "Revoke the caller's own admin session",
...
const document = new OpenApiGeneratorV3(registry.definitions).generateDocument({
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "REST surface of apps/api. The public product catalog is served over GraphQL instead (POST /graphql).",
