...
// Not part of CI (no pass/fail gate is defined for perf numbers yet — see
// docs/performance-review.md) — run manually with `pnpm --filter api
// load-test` against a local or staging deployment before any change that
// touches hot paths (catalog query, outfit save).
...
async function main() {
  ...
  console.info(
    `NOTE: apps/api rate-limits write endpoints (default ${process.env.RATE_LIMIT_MAX_REQUESTS ?? 30} req/${process.env.RATE_LIMIT_WINDOW_SECONDS ?? 60}s per IP, see docs/07-security.md) — these read-only scenarios avoid that limiter deliberately.`,
  );
